...
class App extends Component {
    ...
    handleOpenModal =  (event,message,message2,message3,message4,message5,message6,message7,message8,message9) => {
    ...
    consulta = async (event) => {
        ...
        const registros = await this.state.contract.methods.getRecords(this.state.cuenta).call();
              ...
              <div key={diagnostico}>
                ...
                {this.state.registrosPorDiagnostico[diagnostico].map((item,j) => (
                    <Typography><lo key={j}>
                        ...
                        Estado: {item.state} 
                        <Stack sx={{ mt: 2, justifyContent: "end"  }} direction="row" spacing={2}>
                            <Button variant="contained" key={j} onClick={(event) => {this.elegir(event,this.setState({ind:j}),this.setState({diag2:diagnostico}));
